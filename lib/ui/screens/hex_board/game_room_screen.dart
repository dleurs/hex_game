...
class _GameRoomScreenState extends BaseScreenState<GameRoomScreen> {
  @override
...
class Board extends Object with IterableMixin<BoardPoint> {
  ...
  Point<double> boardPointToPoint(BoardPoint boardPoint) {
    return Point<double>(
      sqrt(3) * hexagonRadius * boardPoint.q + sqrt(3) / 2 * hexagonRadius * boardPoint.r + size.width / 2,
  ...
  Board copyWithBoardPointColor(BoardPoint? boardPoint, Color color) {
    ...
    final boardPointIndex =
        _boardPoints.indexWhere((boardPointI) => boardPointI.q == boardPoint?.q && boardPointI.r == boardPoint?.r);
    ...
    if (nextBoardPoint != null) {
      nextBoardPoints[boardPointIndex] = nextBoardPoint;
    }
    final selectedBoardPoint = boardPoint == selected ? nextBoardPoint : selected;
    return Board(
      boardRadius: boardRadius,
      hexagonRadius: hexagonRadius,
      hexagonMargin: hexagonMargin,
